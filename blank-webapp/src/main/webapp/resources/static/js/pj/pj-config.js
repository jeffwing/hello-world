...
Vue.config.menu = {
  ...
  filterMenu： 自定义菜单过滤方法，参数为菜单项和过滤文本内容。
}
...
keepCache      设置为true，永久keep-alive，如需刷新，需手动控制数据
...
Vue.config.menu = VueUtil.merge({
  viewsPath: resourceContextPath + "/views",
  data: function (username, roles) {
    ...
    return new Promise(function(resolve, reject) {
        Vue.http.post(
          contextPath + '/user/authMenu.json'
        ).then(function(response) {
          // 处理数据，返回
  ...
  getUserInfo: function(token) {
    ...
    return new Promise(function(resolve, reject) {
        Vue.http.post(
          contextPath + '/user/info.json'
        ).then(function(response) {
            store.state.user = response.data;
          // 处理数据，返回
          resolve({
            "name":response.body.userCode,
            "avatar": resourceContextPath+'/static/img/avatar/1.png'
          });
        }, function(error) {
          // 错误处理
          console.log(error)
...
//var userMap = {
//  admin: {
//    avatar: './resources/images/avatar/1.png',
//    name: 'Super Admin'
//  }
//}